...
- `includes/class-noodled-db.php` — 8 tables via dbDelta: notebooks (`drop_to`, `sort_order`, `color`), notes, users, permissions, note_permissions, attachments (`exif`, `alt`, `sort_order`), reminders (`note_id`, `user_id`, `remind_at`, `label`, `sent`), revisions (`note_id`, `title`, `body`, `created_at` — durable version history, pruned to 15/note)
...
- `includes/class-noodled-rest.php` — 30+ REST endpoints under `noodled/v1`, incl. the private file proxy. Notes ship server-built `preview`+`tasks` (not full bodies); lazy `/bodies` + `/backlinks`; attachment `reorder`/`alt`; notebook `color`; per-user note `reminders` (GET/POST/DELETE/sent); note `revisions` (GET, server-snapshotted on save). `sync_status` exposes GitHub config only to admins. Every response sends `Cache-Control: private, no-store` + `Vary: Cookie` (the client also cache-busts GETs)
...
- Notebooks (+ color), Notes, Bodies, Backlinks, Trash, Search, Attachments (+ reorder/alt), Reminders (per-user, note-attached), File proxy, Import (Evernote/zip), Export, Config, Sync, Plaud, Auth, Sharing, Admin (users/drop/pin/permissions)
