...
- `includes/class-noodled-db.php` — 8 tables via dbDelta: notebooks (`drop_to`, `sort_order`, `color`, `parent_id` — nesting), notes, users, permissions, note_permissions, attachments (`exif`, `alt`, `sort_order`), reminders (`note_id`, `user_id`, `remind_at`, `label`, `sent`), revisions (`note_id`, `title`, `body`, `created_at` — durable version history, pruned to 15/note)
