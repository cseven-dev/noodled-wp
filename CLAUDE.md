...
- `includes/class-noodled-db.php` — 7 tables via dbDelta: notebooks (`drop_to`, `sort_order`, `color`), notes, users, permissions, note_permissions, attachments (`exif`, `alt`, `sort_order`), reminders (`note_id`, `user_id`, `remind_at`, `label`, `sent`)
...
- `includes/class-noodled-rest.php` — 30+ REST endpoints under `noodled/v1`, incl. the private file proxy. Notes ship server-built `preview`+`tasks` (not full bodies); lazy `/bodies` + `/backlinks`; attachment `reorder`/`alt`; notebook `color`; per-user note `reminders` (GET/POST/DELETE/sent). Every response sends `Cache-Control: private, no-store` + `Vary: Cookie` (the client also cache-busts GETs)
